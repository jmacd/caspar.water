...
<div class="hero">
  ...
  <div class="card">${
    resize((width) => Plot.plot({
      title: "Caspar Water Well Depth",
      subtitle: "Caspar Orchard Road",
      width,
	  x: {type: "time", domain: [new Date(2022, 7, 1), new Date(2024, 4, 1)], grid: true},
      y: {grid: true, label: "Meters"},
      marks: [
        Plot.ruleY([0]),
        Plot.lineY(welldepth, {x: "Time", y: "Depth", tip: true})
      ]
    }))
  }</div>
...
```js
const welldepth = FileAttachment("welldepth.csv").csv({typed: true});
